...
class AddViewController: BaseViewController {
    ...
    @objc func saveButtonClicked() {
        ...
        let task = DiaryTable(diaryTitle: titleTextField.text!, diaryDate: Date(), diaryContents: contentTextView.text, diaryPhoto: userImageUrl)
        
        repository.createItem(task)
        
        // 이미지 저장
        if userImageView.image != nil {
            saveImageToDocument(fileName: "mar_\(task._id).jpg", image: userImageView.image!)
        }
